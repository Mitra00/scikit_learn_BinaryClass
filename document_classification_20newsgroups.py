...
data_train_size_mb = size_mb(data_train.message)
data_test_size_mb = size_mb(data_test.message)

print("%d documents - %0.3fMB (training set)" % (
    len(data_train.message), data_train_size_mb))
print("%d documents - %0.3fMB (test set)" % (
    len(data_test.message), data_test_size_mb))
...
if opts.use_hashing:
    ...
    X_train = vectorizer.transform(data_train.message)
else:
    vectorizer = TfidfVectorizer(sublinear_tf=True, max_df=0.5,
                                 stop_words='english')
    #X_train = vectorizer.fit_transform(data_train.data)
    X_train = vectorizer.fit_transform(data_train.message)
duration = time() - t0
...
X_test = vectorizer.transform(data_test.message)
duration = time() - t0
